...
**Interfaces:**
- Produces: `Tenant` 엔티티 클래스 — `id`, `slug`, `name`, `schemaName`, `planType`, `status`, `createdAt`, `updatedAt`, `deletedAt` 필드
...
export class Tenant {
  ...
  // 화면에 표시하는 테넌트 이름 (예: "Acme Corporation")
  @Column({ length: 100 })
  name: string;
  ...
  // schemaName: 이 테넌트의 실제 PostgreSQL 스키마 이름.
  // slug와 별도 저장하는 이유: slug가 나중에 바뀌어도 실제 데이터 저장 위치(스키마)는 유지된다.
  @Column({ name: 'schema_name', unique: true, length: 100 })
  schemaName: string;

  // 구독 플랜 타입. DEFAULT 'FREE'이므로 INSERT 시 생략 가능.
  @Column({ name: 'plan_type', length: 20, default: 'FREE' })
  planType: string;

  // 테넌트 상태. 'ACTIVE' | 'SUSPENDED' 등으로 관리.
  ...
  // 이후 find() 쿼리에서 WHERE deleted_at IS NULL 조건이 자동으로 붙는다.
...
DDL은 `database/ddl/schema.sql`에 이미 작성됨. 아래 구조 확인:

```sql
-- public.tenants (전체 컬럼)
...
-- 테넌트별 스키마(tenant_{slug})의 테이블들은 schema.sql에 전체 정의됨
```
